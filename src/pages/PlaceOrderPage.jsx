...
const PlaceOrderPage = () => {
    ...
    const handlePurchase = () => {
        ...
        Object.keys(localStorage).forEach((key) => {
            let match = key.match(/quantityCount_(\d+)/);
            if (match) {
                let num = match[1];
                let quantityCount = localStorage[`quantityCount_${num}`];
                let cartPrice = localStorage[`cartPrice_${num}`];
                let shopName = localStorage[`shopName_${num}`];
                let productName = localStorage[`productName_${num}`];
                let productImage = localStorage[`productImage_${num}`];
                let classification = localStorage[`classification_${num}`];

                if (localStorage[num] === "true" && quantityCount && cartPrice) {
                    ...
                    // Tìm xem cửa hàng đã có trong formattedData hay chưa
                    let shopIndex = formattedData.findIndex(
                        (shop) => shop.shop_name === shopName
                    );

                    if (shopIndex !== -1) {
                        // Nếu cửa hàng đã tồn tại, thêm sản phẩm vào Products
                        formattedData[shopIndex].Products.push(itemData);
                    } else {
                        // Nếu cửa hàng chưa tồn tại, thêm cửa hàng mới vào formattedData
                        formattedData.push({
                            shop_name: shopName,
                            Products: [itemData],
                        });
                ...
                <div class="row">
                    <div class="col d-flex align-items-center">
                        <a className="reset_a" href="/">
                            <div class="logo d-flex align-items-end justify-content-center">
                                ...
                                <div className="text-center h3 m-0">Thanh toán</div>
                            </div>
                        </a>
                    </div>
                </div>
        ...
        <body className="body">
            <section className="heading-title my-4 container">
                <div class="heading-title-content px-4 pt-3 pb-3 py-3 px-4 rounded shadow-sm">
                    <div class="row" >
                        <div class="col-md-4">
                            <div class="p_color h5"><MapPin /> Địa chỉ nhận hàng</div>
                        </div>
                    ...
                    <div class="row mt-3">
                        ...
                        <div class="col-md-6">
                            <div class="customer_address">Số 192, tổ 7, thôn Xuân Thượng, xã Lạc Lâm, huyện Đơn Dương, tỉnh Lâm Đồng</div>
                        </div>
                        <div class="col-md-3 d-flex justify-content-end">
                            <div class="customer_manipulate">
                                <button className="button_with_p_color" onClick={() => { alert(JSON.stringify(formattedData)) }}>
                                    Đổi địa chỉ khác
                                </button>
                            </div>
                        </div>
                    ...
                    <ul class="row list-unstyled align-items-center pt-4 pb-4">
                        <li class="col">
                            <div class="z_checkbox px-3">
                                <label class="form-check-label" for="check_buy_all">Sản phẩm</label>
                            </div>
                        </li>
                        <div class="col">
                        ...
                        <div className="item_in_cart  py-3 px-4 rounded shadow-sm">
                            <ul class="row list-unstyled align-items-center pt-4 pb-4">
                                <li class="col">
                                    <div class="z_checkbox  px-3">
                                        <a class='reset_a'>
                                            <label class="form-check-label d-flex align-items-center reset_a" for="check_buy_all">{shop.shop_name} <img
                                                ...
                                                style={{ width: '18px', height: '18px', borderRadius: '5px', marginLeft: '3px' }}
                                            /></label>
                                        </a>
                                    </div>
                                </li>
                                                ...
                                                <ul class="item_in_shop row list-unstyled align-items-center pt-3">
                                                    ...
                                                    <div class="col">
                                                        <ul class="row list-unstyled">
                                                            <li class="col text-center price">{(item.totalPrice / item.quantityCount).toLocaleString('vi-VN')} ₫</li>
                                                            <li class="col text-center ">{item.quantityCount}</li>
                                                            <li class="col text-center total_price">{parseInt(item.totalPrice).toLocaleString('vi-VN')} ₫</li>
                                                        </ul>
                                                    </div>
                                                </ul>

                                                {!isLastElement ? <div className="divider_in_row mt-4 mb-2"></div> : <div></div>}
                                            </>
                                        );
                                    })
                                }
                ...
                <div className="container mt-4 bg-white py-3 px-4 rounded shadow-sm">
                    ...
                    <div class="row">
                        <div class="col-md-6 d-flex align-items-center">
                            Nhấn "Xác nhận đặt hàng" đồng nghĩa với việc bạn chấp nhận Điều khoản <a href="" className="px-1">Zimara</a>
                        </div>
                        <div class="col-md-6 text-end mt-3 mb-3">
                            <Link class="btn btn-primary purchase_btn reset_a" to="/PlaceOrderPage">Xác nhận đặt hàng</Link>
                        </div>
                    </div>
